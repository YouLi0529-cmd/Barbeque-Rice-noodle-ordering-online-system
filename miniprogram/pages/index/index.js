...
Page({
  data: {
    ...
    tableNumber: '', // 桌码号
    orderMode: 'dineIn', // 当前点餐模式
    recommendedDishes: [], // 首页推荐菜
    fallbackRecommendations: [
      { name: '火盆牛肉', price: 38, description: '炭火现烤，油香足' },
      { name: '五花肉', price: 32, description: '肥瘦相间，适合慢烤' },
      { name: '掌中宝', price: 36, description: '脆口下酒，朋友局必点' },
      { name: '烤苕皮', price: 12, description: '重庆烧烤灵魂配菜' },
      { name: '冰粉', price: 8, description: '解辣清爽，饭后刚好' },
      { name: '啤酒', price: 8, description: '炭火局的热闹底色' }
    ],
    // 菜品分页
    goodsPage: 0,
    goodsPageSize: 20,
  ...
  // 兼容旧 scene 桌码与新 mode/tableNumber 参数
  applyEntryOptions(options = {}) {
    const entry = this.parseEntryOptions(options)
    if (entry.mode === 'dineIn') {
      this.setData({
        orderMode: 'dineIn'
      })
    }
  ...
  parseEntryOptions(options = {}) {
    ...
    if (options.scene) {
      try {
        ...
        const sceneParams = this.parseQueryLikeString(scene)
        if (sceneParams.mode) {
          result.mode = sceneParams.mode
        }
        if (sceneParams.tableNumber) {
          result.tableNumber = sceneParams.tableNumber
        } else if (sceneParams.scene) {
          result.tableNumber = sceneParams.scene
        } else if (scene && scene.indexOf('=') === -1) {
          result.tableNumber = scene.trim()
        }
      } catch (e) {
    ...
    if (result.tableNumber) {
      result.tableNumber = decodeURIComponent(result.tableNumber).trim()
    }

    return result
  },

  parseQueryLikeString(str = '') {
    const params = {}
  ...
  goOutdoorEntry() {
    wx.navigateTo({
      url: '/pages/outdoor/outdoor?mode=outdoor&outdoorPointId=main'
    })
  },
  ...
  scanTableCode() {
    ...
    wx.scanCode({
      ...
      success: (res) => {
        ...
        // 从 path 参数中提取桌码号，兼容 scene 和 tableNumber
        if (res.path) {
          const queryStr = res.path.split('?')[1]
          if (queryStr) {
            const params = this.parseQueryLikeString(queryStr)
            if (params.tableNumber) {
              tableNumber = params.tableNumber
            } else if (params.scene) {
              const sceneParams = this.parseQueryLikeString(decodeURIComponent(params.scene))
              tableNumber = sceneParams.tableNumber || sceneParams.scene || params.scene
            }
          }
        }
    ...
    try {
      ...
      const doBuyRes = await wx.cloud.callFunction({
        ...
        data: {
          ...
          orderType: this.data.tableNumber ? 'dineIn' : 'takeOut'
        }
      })
